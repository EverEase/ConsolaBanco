...
public class Consola {
	...
	//inicializamos lso atributos en el constructor
	Consola(){
		...
		this.idOrdenante = (int) (Math.random()*10);
...
private Cliente buscarClientePorId(int idCliente) {
	...
	if (clienteResultado != null) {
		...
		System.out.println("[i] ID Gestor");
		System.out.println("[x] Cancelar");
		System.out.print("Campo a actualizar: ");
		char opcionActualizar = teclado.next().charAt(0);
		switch (opcionActualizar) {
  ...
  private void consultarTransferencia() {
	  ...
	  private void enviarTransferencia() {
		  System.out.print("ID Beneficiario ");
			...
			Transferencia nuevaTransferencia = new Transferencia(siguienteIdTransferencia, idOrdenante, idBeneficiario, importe, concepto);
				transferencias.add(nuevaTransferencia);
				siguienteIdTransferencia++;
				System.out.println("Transferencia enviada con exito");
